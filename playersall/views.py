...
def playersall(request):
    if request.method == "GET":
        ...
        sql = "SELECT * FROM (SELECT * FROM PERSON P1, PLAYER P2  WHERE P1.PERSON_ID=P2.PLAYER_ID) P LEFT OUTER JOIN PLAYER_STAT PS ON (P.PLAYER_ID=PS.PLAYER_ID) LEFT OUTER JOIN STAT S ON (PS.STAT_ID=S.STAT_ID) ORDER BY S.RATING DESC NULLS LAST"
        cursor.execute(sql)
        ...
        if str(name) in players_fullname:
            ...
            cursor.execute(
                "SELECT * FROM PERSON P, PLAYER PL WHERE P.PERSON_ID=PL.PLAYER_ID AND FIRST_NAME=:FNAME AND LAST_NAME=:LNAME",
            ...
            for r in result:
                ...
                row = {'id': id, 'first_name': first_name, 'last_name': last_name, 'full_name': full_name,
                       ...
                       'batstyle': bat_style, 'bowlstyle': bowl_style}
                dict_result.append(row)

            print(dict_result)
            cursor.execute("SELECT * FROM PLAYER_STAT PS, STAT S WHERE PLAYER_ID=:PID AND PS.STAT_ID=S.STAT_ID",
                           PID=pid)
            ...
            if len(result21) != 0:
                ...
                playerSTAT = {'scored_run': scored_run, 'motm': motm, 'hundred': hundred, 'fifty': fifty,
                              'notout': notout, 'avgrun': avgrun, 'strate': strate, 'wickets': wickets,
                              'fifer': fifer, 'overs': overs, 'givenrun': givenrun, 'bowl_avg': bowl_avg,
                              'bowl_strate': bowl_strate, 'matches': matches, 'rating': rating}
            ...
            for r in res:
                ...
                row = {'mid': mid, 'scored_run': scored_run, 'balls': balls, 'fours': fours, 'sixes': sixes,
                       'notout': notout,
            ...
            return render(request, 'playersingle/playersingle.html', {'name': name, 'details': dict_result[0],
                                                                      'playerstat': playerSTAT, 'matches': Matches,
                                                                      'age': fun_age})
        else:
